...
export class CupDetailsComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  constructor(public router: Router, public socketService: SocketService,
     public loader: NgxSpinnerService, private service: MatchService,
      private ac: ActivatedRoute, private location: Location,
  ...
  getMatchDetails(matchId, marketId) {
    ...
    this.service.getMatchDetails(payload).subscribe((res) => {
      ...
      if (res?.status) {
        ...
        self.sesion_market  = self.matchDetails?.fancies.filter(item => item.fancy_type_id == '1' || item.fancy_type_id == '2');
        ...
        console.log(this.matchDetailsFancy1)
        console.log(this.matchDetailsOddEven)
