...
export class CasinoMatchDetailsComponent  implements OnInit, OnDestroy, AfterViewInit  {
  selectedIndex2: number = -1;
  sportsId:any;
  selectedIndex: any = 'Fancy1';

  constructor(public router: Router, public socketService: SocketService, public loader: NgxSpinnerService,
     private service: MatchService, private ac: ActivatedRoute, private location: Location, public toaster: ToastrService, private sanitizer: DomSanitizer, private _ngZone: NgZone, private ref: ChangeDetectorRef) { }
  cardResult: any;
  cardResultArray: any;
  marketname: any
      ...
      if (res?.status) {
        ...
        if(this.matchDetails.match_id =='-1027' || this.matchDetails.match_id =='-1028'){
          ...
          if(this.chartData != undefined && this.chartData != null && this.chartData !=''){
            ...
            this.service.setPieChartData(chartValues)
          }else{
            this.service.setPieChartData(this.staticArray)
          }
        
        }
        this.sportId=self.matchDetails.sport_id;
        this.fancyDetails1= this.matchDetails.fancies.filter(item => item.fancy_type_id =='2')
        this.fancyDetailsOddEven= this.matchDetails.fancies.filter(item => item.fancy_type_id =='3')
        this.fancyDetails2= this.matchDetails.fancies.filter(item => item.fancy_type_id =='4')
  ...
  suspendedPoker6Class(selectionStatus) {
  ...
  suspendedClass(selectionStatus, marketStatus) {
    ...
    if ((selectionStatus == 'ACTIVE' || selectionStatus == 'OPEN' || selectionStatus == '1' || selectionStatus == 'True' || selectionStatus == 'true') || (marketStatus == 'ACTIVE' || marketStatus == 'OPEN' || marketStatus == '1' || marketStatus == 'True' || marketStatus == 'true')) {
      return '';
    }
    else {
      return 'suspended';
  ...
  suspendedClassOne(selectionStatus) {
  ...
  suspendedTextDTL(selectionStatus) {
