...
export class OtpComponent implements OnInit {
  ...
  constructor(private renderer: Renderer2,private fb:FormBuilder,private service:AuthService,private route:Router ,private toaster:ToastrService,private userservice:UserService) { }
  loginForm: FormGroup
  logo;
  isSignUp:any='0'
  userId:any;
  ...
  // console.log(this.loginForm.invalid ,"invalid", this.loginForm.value)
    if (this.loginForm.invalid) {
     ...
     let otp=  this.loginForm.value.otp1 + this.loginForm.value.otp2 +this.loginForm.value.otp3 +this.loginForm.value.otp4 +this.loginForm.value.otp5 +this.loginForm.value.otp6
    let data ={
      'user_id': +this.userId,
      'otp':otp.toString(),
    }
