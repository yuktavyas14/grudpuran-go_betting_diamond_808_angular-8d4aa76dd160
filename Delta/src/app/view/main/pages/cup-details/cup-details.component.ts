...
export class CupDetailsComponent implements OnInit {
  ...
  getMatchDetails(matchId: any, marketId: any) {
    ...
    this.matchService.getMatchDetails(payload).subscribe((res) => {
      if (res?.status) {
        if (res?.data != null)
          ...
          if(this.matchDetails.fancies.length > 0){
            ...
            this.sessionMarketFancy= this.matchDetails.fancies.filter((item:any)=> ((item.fancy_type_id ==1 || item.fancy_type_id==2)));
      ...
      (err) => {
        if (this.location?.path().split('/')[2] === 'cupdetails') {
