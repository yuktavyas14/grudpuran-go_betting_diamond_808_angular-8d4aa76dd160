...
export class MatchDetailComponent implements OnInit, OnDestroy {
  ...
  slideConfigAndar = {"slidesToShow": 5, "slidesToScroll": 1};
  slideConfigBahar = {"slidesToShow": 5, "slidesToScroll": 1};
  ...
  constructor(public dialog: MatDialog,private socketService: SocketService, public shorterNumber: ShortNumberPipe,
    ...
    private toaster: ToastrService, private sanitizer: DomSanitizer, private router: Router,) {
      ...
      .subscribe(params => {
        this.isCup = params.cup;
        console.log(this.isCup); // price
      }
    );
      this.sportId = sessionStorage.getItem('sportId')
      this.isCupMatch = localStorage.getItem('isCup')
    ...
    this.matchService.getMatchDetails(payload).subscribe((res) => {
      if (res?.status) {
        if (res?.data != null)
          ...
          if(this.matchDetails.match_id =='-7'){
            this.andarCards = self.matchDetails.markets[0].runner_json.filter((item:any)=>((item.selection_id > 0 && item.selection_id <= 13)))
            this.baharCards = self.matchDetails.markets[0].runner_json.filter((item:any)=>((item.selection_id > 20 && item.selection_id <= 33)))
          }
  ...
  showMinMaxToolTip(selectionId:any,matchId: any, isValid: any) {
    this.isMinMax = !this.isMinMax
    if (this.isMinMax) {
  ...
  placeMarketBet() {
    ...
    this.matchService.placeMarketBet(this.betdetails).subscribe((res:any) => {
      this.loader.hide()
    this.loading= false;
  ...
  setFancyPosition(fancy:any,data: any, index: number) {
    this.fancyPostion = [];
    console.log(data)
    console.log(fancy)
    this.indexFancy = index
    if (data)
  ...
  isShowAllCards(selectionStatus:any) {
  ...
  getImagename(value:any){
    ...
    if(value =='11' || value =='31'){
      return '11'
    }
    if(value =='12' || value =='32'){
      return '12'
    }
    if(value =='13' || value =='33'){
      return '13'
    }
    return '0'
  ...
  suspendedIconAll( marketStatus: any) {
  ...
  suspendedClassAll( marketStatus: any) {

    if ((marketStatus == 'ACTIVE' || marketStatus == 'OPEN' || marketStatus == '1' || marketStatus == 'True')) {
      return '';
    }
    else {
      return 'number-10-disabled';
    }
  }
  ...
  showResults(result: any) {
    ...
    this.matchService.getCasinoResult(marketData).subscribe(response => {
      if (!response.error) {
  ...
  addAppointment() {
    console.log('ajgsdjhgsahd')
    // this.addAppointments= !this.addAppointments;
    // this.uploadPrescription= false;
    // this.filterAppointment=false;
    // this.healthRecord= false;
    const dialogRef = this.dialog.open(AndarBaharResultComponent, {
      width: '100%',
      height: '100',
      // position: {
      //   top: '75px',
