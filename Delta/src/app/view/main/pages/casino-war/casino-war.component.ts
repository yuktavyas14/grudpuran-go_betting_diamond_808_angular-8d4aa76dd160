...
@Component({
  ...
  styleUrls: ['./casino-war.component.scss'],
...
export class CasinoWarComponent implements OnInit {
  home: any;
  away: any;
  ballByBallScore: any;
  matchId: any;
  ...
  constructor(private socketService: SocketService, public shorterNumber: ShortNumberPipe,
    ...
    private activatedRoute: ActivatedRoute, private matchService: MatchService,
  ...
  getMatchDetails(matchId: any, marketId: any) {
    ...
    this.matchService.getMatchDetails(payload).subscribe((res) => {
      if (res?.status) {
        if (res?.data != null)
          ...
          if(this.matchDetails.match_id =='-1029'){
            ...
            let SecondData= runnerJson.filter((item:any)=> item.selection_id >9 && item.selection_id <=14);
            let  SecondData1= runnerJson.filter((item:any)=> item.selection_id >14 && item.selection_id <=18);

            let ThirdData= runnerJson.filter((item:any)=> item.selection_id >18 && item.selection_id <=23);
            let ThirdData1= runnerJson.filter((item:any)=> item.selection_id >23 && item.selection_id <=27);
            
            let FourData= runnerJson.filter((item:any)=> item.selection_id >27 && item.selection_id <=32);
            let  FourData1= runnerJson.filter((item:any)=> item.selection_id >32 && item.selection_id <=36);
            
            let FiveData= runnerJson.filter((item:any)=> item.selection_id >36 && item.selection_id <=41);
            let FiveData1= runnerJson.filter((item:any)=> item.selection_id >41 && item.selection_id <=45);
            
            let SixData= runnerJson.filter((item:any)=> item.selection_id >45 && item.selection_id <=14);
            let SixData1= runnerJson.filter((item:any)=> item.selection_id >14 && item.selection_id <=54);


            console.log(this.firstRowData,'firstData')
            console.log(this.firstRowData1,'firstData1')
          }
      ...
      (err) => {
        if (this.location?.path().split('/')[2] === 'casino-war') {
