...
@Component({
  selector: 'app-deleted-bet-history',
  templateUrl: './deleted-bet-history.component.html',
  styleUrls: ['./deleted-bet-history.component.css'],
  providers: [DatePipe, ExcelServiceService]
})
export class DeletedBetHistoryComponent implements OnInit {
  sportList      : any = []
...
searchKeyValue:any=''
  ...
  ngOnInit(): void {
    ...
    this.searchKey.valueChanges.subscribe(value => {
      ...
      this.getDeleteBetStatementReport(this.page ,value)
    });
...
this.getDeleteBetStatementReport(this.page,'');
  ...
  getDeleteBetStatementReport(page,value) {
    ...
    this.service.deletedBetHistory(payload).subscribe((res)=>{
      if(res?.status){
  ...
  pageChange(page) {
    ...
    this.getDeleteBetStatementReport(this.page,this.searchKeyValue)
  }
