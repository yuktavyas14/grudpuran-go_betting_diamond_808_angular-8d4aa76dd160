...
declare const $:any;
@Component({
  selector: 'app-payment-deposit-list',
  templateUrl: './payment-deposit-list.component.html',
  styleUrls: ['./payment-deposit-list.component.css'],
  providers: [DatePipe, ExcelServiceService]
})
export class PaymentDepositListComponent implements OnInit {
...
this.getPaymentDepositList(this.page);
  }
  private convert(str) {
  ...
  getPaymentDepositList(page) {

    let payload = {
      page: page
    }

    this.service.getPaymentDepositList(payload).subscribe((res)=>{
      if(res?.status){
        this.listBetData= res.data.data
  ...
  acceptDepositRequest(item,status) {
    ...
    this.service.acceptDepositRequest(payload).subscribe((res)=>{
      if(res?.status){
        this.getPaymentDepositList(this.page);
      }else{
